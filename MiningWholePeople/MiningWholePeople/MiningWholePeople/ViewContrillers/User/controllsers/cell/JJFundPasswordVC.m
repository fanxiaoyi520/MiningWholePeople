...
@interface JJFundPasswordModel : ZDPayRootModel

@property (nonatomic ,copy)NSString *payword;
@property (nonatomic ,copy)NSString *confirmpwd;
@end

@implementation JJFundPasswordModel

@end
@interface JJFundPasswordVC ()

@property (nonatomic ,assign)CGFloat sure_top;
@property (nonatomic ,strong)NSMutableDictionary *mutableDic;
@property (nonatomic ,strong)JJFundPasswordModel *fundPasswordModel;
@end

@implementation JJFundPasswordVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.naviTitle = GCLocalizedString(@"资金密码");
...
- (void)init_UI
{
    NSArray *fundArray = @[GCLocalizedString(@"请输入您要设置的资金密码"),GCLocalizedString(@"请再次输入您的新资金密码")];
...
{
    JJFundPasswordModel *model = [JJFundPasswordModel mj_objectWithKeyValues:self.mutableDic];
    self.fundPasswordModel = model;
    
    if ([model.payword isEqualToString:@""]) {
        [self showMessage:GCLocalizedString(@"请输入6位数字!") target:nil];
        return;
    }
    
    if (model.payword.length != 6) {
        [self showMessage:GCLocalizedString(@"请输入6位数字!") target:nil];
        return;
    }
...
- (void)networking_set_pay_pwd
{
    ...
    NSString *payword = [NSString stringWithFormat:@"%@",self.fundPasswordModel.payword];
    NSString *confirmpwd = [NSString stringWithFormat:@"%@",self.fundPasswordModel.confirmpwd];
    NSDictionary *params = @{@"payword":payword,@"confirmpwd":confirmpwd};
    [AFNetworkingManager requestWithType:SkyHttpRequestTypePost urlString:[NSString stringWithFormat:@"%@%@",DOMAINNAME,SET_PAY_PWD] parameters:params successBlock:^(id  _Nonnull responseObject) {
        ...
        if ([code isEqualToString:@"1"]) {
            if (self.fundPasswordBlock) {
                self.fundPasswordBlock([responseObject objectForKey:@"msg"]);
            }
